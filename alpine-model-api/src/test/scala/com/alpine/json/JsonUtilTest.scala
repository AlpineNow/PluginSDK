...
class JsonUtilTest extends FunSuite {
  ...
  test("Should serialize registered types correctly.") {
    val gson = JsonUtil
      .prettyGsonWithTypeHints(
        KnownTypes(Map(classOf[java.lang.Double] -> "Double", classOf[java.lang.Boolean] -> "Boolean"))
      )
    val anyValues: List[TypeWrapper[Any]] = List[Any](1.3, true).map(a => TypeWrapper[Any](a))
    val anyListType = new TypeToken[List[TypeWrapper[Any]]] {}.getType
    val stringJson = gson.toJson(anyValues, anyListType)
