...
import com.alpine.json.TypeWrapper
...
case class RealValuedFunctionsModel(functions: Seq[RealFunctionWithIndex], inputFeatures: Seq[FeatureDesc[_]]) extends RowModel {
  override def transformer = RealValuedFunctionTransformer(this)

  override def outputFeatures = {
    functions.map(f => f.function.value.getClass.getSimpleName + "_" + inputFeatures(f.index).name).map(name => FeatureDesc(name, DoubleType()))
  }
}
